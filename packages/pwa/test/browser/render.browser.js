...
describe('app mounts and reacts in a real browser', () => {
  ...
  it('shows the pairing hero for a fresh visitor, then the conversation when tethered', async () => {
    ...
    assert(await waitFor(() => root.querySelector('.hero')), 'pairing hero not shown for a fresh visitor');
    assert(root.textContent.includes('Scan the QR'), 'pairing prompt missing');
    ...
    assert(await waitFor(() => root.querySelector('.status .dot.tethered')), 'status dot not in tethered state');
