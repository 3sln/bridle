...
export default alias(function (state) {
  ...
  return div({ className: 'sheet-backdrop' },
    div({ className: 'sheet' },
      div({ className: 'sheet-head' }, span('Sessions'), button({ className: 'btn ghost' }, 'Done').on({ click: close })),
      button({ className: 'btn big' }, '＋ New session').on({ click: () => fire('new-session') }),
      list.length
        ...
        : p({ className: 'hint' }, 'No earlier sessions for this project yet.'),
